...
class TemporalSelfAttention(nn.Module):
    ...
    def forward(self, Q, B_prev):
        ...
        feat = torch.cat([Q, B_prev], dim=-1)           # [B, H*W, 2C]
        offsets = self.offset_proj(feat)                 # [B, H*W, n_heads*2*n_points]
        weights = self.attn_weight_proj(feat).softmax(-1)# [B, H*W, n_heads*n_points]
...
class SpatialCrossAttention(nn.Module):
    def __init__(self, embed_dim=256, n_heads=8, n_levels=3,
        ...
        self.n_ref = n_ref
        self.n_points = n_points
        # 高度锚点：N_ref 个均匀采样高度（-5m 到 3m）
        z_anchors = torch.linspace(z_min, z_max, n_ref)
        self.register_buffer('z_anchors', z_anchors)    # [N_ref]
        # BEV 格子中心的真实世界坐标（ego-centric）
        # x'=(xi-W/2)*s,  y'=(yi-H/2)*s
        xi = torch.arange(bev_w).float()
        yi = torch.arange(bev_h).float()
        gy, gx = torch.meshgrid(yi, xi, indexing='ij')
        bev_x = (gx - bev_w / 2) * s                   # [H, W]
        bev_y = (gy - bev_h / 2) * s                   # [H, W]
        bev_xy = torch.stack([bev_x, bev_y], dim=-1).reshape(-1, 2)
        self.register_buffer('bev_xy', bev_xy)          # [H*W, 2]
        # 可学习层
        self.value_proj = nn.Linear(embed_dim, embed_dim)
        self.out_proj   = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, Q, img_feats, proj_matrices):
        ...
        # ② 投影到各摄像头图像
        # T_i: [B, N_view, 3, 4]  ×  ref_3d: [HW, N_ref, 4, 1]
        # → 2D 像素坐标 [B, N_view, HW, N_ref, 2]  (u/z, v/z)
        ref_3d_batch = ref_3d_h.unsqueeze(0).unsqueeze(0).expand(B, N_view, -1, -1, -1)
        proj = torch.einsum('bvij,bhwj->bvhwi', proj_matrices,
...
class BEVFormerLayer(nn.Module):
    ...
    def forward(self, Q, B_prev, img_feats, proj_matrices):
        ...
        Q = self.norm1(Q + self.tsa(Q, B_prev))
...
class BEVFormer(nn.Module):
    ...
    def forward(self, img_feats, proj_matrices, B_prev=None):
        ...
        if B_prev is None:
            ...
            B_prev = ego_motion_align(B_prev, ...)            # 推断：用自车运动 warp
...
- **[DETR](detr-2005.12872.md)**：BEVFormer 检测头基于 Deformable DETR，用匈牙利匹配 + object queries
...
| 对齐方式 | 用自车在 t-1 → t 的运动（旋转+平移）进行 BEV 平面的仿射变换 |
| 训练时处理 | 对 {B_{t-3},B_{t-2},B_{t-1}} 不反传梯度（stop-gradient）|
