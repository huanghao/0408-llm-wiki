...
# 静态意图查询（全程不变，代表"这个 query 负责的运动模式"）
...
for layer in range(6):
    ...
    # 用静态 Q_I 作为位置 embedding，C 作为内容特征
    C_sa = MultiHeadAttn(
        query = C + Q_I,    # [K, D]，位置编码加入内容
        key   = C + Q_I,    # [K, D]
        value = C,          # [K, D]，只聚合内容
    )                       # [K, D]

    # 5b. Cross-Attention to agent features（了解场景中的 agent 状态）
    # 用 [内容特征, 动态 query] 拼接作为 query，查询 agent feature
    C_A = MultiHeadAttn(
        query = concat([C_sa, Q_S], dim=-1),   # [K, 2D]，内容+位置
        key   = concat([A, PE_agent], dim=-1), # [N_a, 2D]，agent+位置
        value = A,                             # [N_a, D]
    )   # [K, D]

    # 5c. Dynamic Map Collection + Cross-Attention（精化轨迹的关键）
    # 收集当前预测轨迹周围最近的 L=128 条 map polyline
    ...
    # 5e. 预测当前层的轨迹（GMM 参数）
    Z = MLP(C)         # [K, T, 6]，6 = (μx, μy, σx, σy, ρ, logit_prob)
    Y = Z[..., :2]     # [K, T, 2]，取均值作为轨迹点

    # 5f. 更新动态搜索 query（指向当前预测的终点，下一层聚焦这片区域）
    Q_S = MLP(sinusoidal_PE(Y[:, -1, :]))   # [K, D]，用终点位置更新

# ── Step 6：输出 ────────────────────────────────────────────────────────
# Z 是最后一层的 GMM 参数
probs = softmax(Z[..., -1])    # [K]，每条轨迹的概率（来自 logit_prob）
trajs = Z[..., :2]             # [K, T, 2]，K 条轨迹的均值点

# 评测时：用 NMS 把 K=64 条轨迹压缩到 6 条
# 训练时：全部 K=64 条参与 WTA loss（找最近的一条做回归+分类 loss）
...
每层 decoder 的输出都计算 $\mathcal{L}_{\text{main}}$（深度监督），各层权重相等。
