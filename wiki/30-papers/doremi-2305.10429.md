...
**"最优"怎么定义**：DoReMi 的"最优"不是针对某个特定任务的最优，而是一个鲁棒最优（[minimax](../20-concepts/minimax.md) 意义上的最优）——**让所有 domain 上的最差表现尽量好**。形式上就是 Group DRO 的 minimax 目标：找一组权重，使得最难的那个 domain 的超额损失最小。
