...
```
大量文本文件（Wikipedia dump，几十 GB）
    ↓
1. 分词（SentencePiece / 空格切词）
   "机器学习很有趣" → ["机器学习", "很", "有趣"]

    ↓
2. 统计所有 n-gram 的出现次数（lmplz 做这一步）
   扫描所有文件，维护一个巨大的计数表：
   ("机器学习", "很") → 12847 次
   ("机器学习", "是") →  8923 次
   ("机器学习", "需") →  3201 次
   ...（5-gram 组合可能有几十亿条）
    ...
    ↓
3. 计算条件概率 + 平滑（Kneser-Ney）
   P("很" | "机器学习") = 12847 / (12847+8923+3201+...) ≈ 0.43
   没见过的 n-gram 用平滑算法给非零值

    ↓
4. 存成 ARPA 格式（.arpa 文件）
   再用 build_binary 压缩成 .arpa.bin（更快加载）

    ↓
语言模型文件（比如 zh.arpa.bin，几百 MB）
```

**语言模型以什么形态存在**：

最终产物是一个 `.arpa.bin` 二进制文件，本质上是一张**压缩的查找表**：

```
key:   (context_tokens..., target_token)  →  一个 n-gram
value: log₁₀ P(target | context)         →  一个 float
```

不是 Python dict——那太慢太大。KenLM 用专门的数据结构（trie + 量化压缩）把几十亿条 n-gram 塞进几百 MB，查询一条 n-gram 只需要几十纳秒。

**推理时做什么**：

```python
import kenlm
model = kenlm.Model("zh.arpa.bin")   # 加载查找表到内存

# 给一行文本打分
score = model.score("机器学习很有趣")
# 内部：把句子切成 bigram/trigram/.../5-gram
# 每个 n-gram 查表得到 log P，全部加起来
# 返回 log₁₀ P(整句)
```

**为什么叫"模型"而不叫"数据库"**：

严格来说它就是一个数据库。之所以叫模型，是因为它对没见过的 n-gram 有"预测能力"——通过平滑算法，它不是简单地返回 0，而是根据更短的 n-gram（回退）给出一个合理估计。这个"泛化到没见过的情况"的能力，是"模型"这个词的核心含义。

## 附录：lmplz 是什么

`lmplz` 是 KenLM 自带的命令行工具，专门用来从大量文本文件训练 n-gram 语言模型。

**基本用法**：

```bash
# 从文本文件训练 5-gram 模型，输出 ARPA 格式
lmplz -o 5 -S 8G -T /tmp < corpus.txt > model.arpa

# 压缩成二进制格式（更快加载，更省内存）
build_binary model.arpa model.arpa.bin
```

参数说明：
- `-o 5`：n-gram 阶数，这里是 5-gram（看前 4 个词预测下一个）
- `-S 8G`：允许使用的内存上限，超出时自动分批写磁盘
- `-T /tmp`：临时文件目录（统计过程会产生大量中间文件）

**它在做什么**：

1. 读取所有文本，统计每种 n-gram（1-gram 到 5-gram）的出现次数
2. 应用 Kneser-Ney 平滑，计算每条 n-gram 的 log P
3. 写成 ARPA 格式（文本，可读）
...
每行三列：`log P(word|context)`、`word`（含 context）、`backoff weight`（回退权重）。

**backoff weight 里"没见过"是什么情况**：这里的"没见过"不是指这个词没见过，而是指**这个 n-gram 的 context 没见过**。比如 5-gram 模型里，查 P("趣" | "我 喜欢 机器 学习") 时，如果训练语料里从来没出现过"我 喜欢 机器 学习"这个 4-gram context，就无法用 5-gram 概率，需要"回退"到 4-gram：P("趣" | "喜欢 机器 学习")。backoff weight 就是做这个回退时的缩放系数，保证所有情况加起来的概率仍然归一化为 1。
