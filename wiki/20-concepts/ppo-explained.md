...
### Q 值、V 值、Advantage 的关系

PPO 里有两个价值概念经常混淆：

**$V(s)$（State Value，状态价值）**：critic 网络预测的"从当前状态出发，按策略走下去能得多少分"。这是 PPO 的 critic 直接输出的东西。

**$Q(s, a)$（Action Value，动作价值）**：在状态 $s$ 执行动作 $a$ 后，继续按策略走下去的期望 Return。PPO 不直接训练 Q 网络，但可以用 return 近似：$Q(s_t, a_t) \approx r_t + \gamma \cdot V(s_{t+1})$（一步 TD 估计）。

**Advantage $A(s, a) = Q(s,a) - V(s)$**：这个动作比"平均动作"好多少。PPO 用 advantage 而不是原始 return 来更新 policy，目的是消除"当前状态本身就好/差"带来的噪声，只保留"这个动作的相对贡献"。

飞船类比：
- $V(s)$：飞船在 t=100 的位置，value 网络预测"平均来说还能得 80 分"
- $Q(s, a)$：在 t=100 **做了完美减速这个动作**，期望得 115 分
- $A$：+35，这个动作比"随便做个动作"好了 35 分
