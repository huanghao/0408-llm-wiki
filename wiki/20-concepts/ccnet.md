...
KenLM 对每行文本调用 `model.score(line)` 得到对数概率（log probability），然后按词数加权汇总：

```
perplexity = 10.0 ** (-doc_log_score / doc_length)
```

其中 `doc_length` = 所有行的词数之和（每行 `len(line.split()) + 1`），`doc_log_score` 是所有行的 log probability 之和。计算前文本会先做归一化：转小写、数字替换为 0、去掉标点。

困惑度本身无量纲，值域是 (1, +∞)。ccNet 按语言分别计算第 30 和第 60 百分位阈值，把数据集切成 head/middle/tail 三段。
...
可运行的 demo 见 `tools/kenlm_perplexity_demo.py`，用纯 Python 实现了相同逻辑，演示了高质量文本 vs 乱码/boilerplate 的困惑度差异，以及 head/middle/tail 分段。

**KenLM 的模型结构和训练方式**：

KenLM 是一个经典的 **n-gram 语言模型**，ccNet 用的是 5-gram（即看前 4 个词预测下一个词）。模型从 Wikipedia 文本训练而来：取 100 万篇 Wikipedia 文章，用 SentencePiece 分词（词表大小 65536），然后用 `lmplz -o 5` 统计所有 5-gram 的频率，最终存成 `.arpa.bin` 二进制格式（每种语言一个模型文件）。

n-gram 语言模型的核心思想非常简单：统计训练语料里每种词序列出现的频率，用频率估算概率。比如 "机器 学习 很 有" 后面跟 "趣" 的概率，就是语料里 "机器 学习 很 有 趣" 出现次数除以 "机器 学习 很 有" 出现次数。没见过的 n-gram 用平滑算法（Kneser-Ney）处理。

**log probability 是什么、怎么算的**：

`model.score(line)` 返回这行文本在语言模型下的 **以 10 为底的对数概率**（log₁₀ P）。对数概率是负数——概率越高，对数越接近 0；概率越低，对数越小（越负）。

比如一句通顺的中文，P ≈ 10⁻²⁰，log₁₀ P ≈ -20；一段乱码，P ≈ 10⁻¹⁰⁰，log₁₀ P ≈ -100。

perplexity 公式 `10^(-log_score / length)` 就是把"每个词的平均对数概率"还原成线性尺度：

```
log_score / length  ≈  每个词的平均 log₁₀ P（负数）
-log_score / length ≈  绝对值（正数）
10^(该值)           =  perplexity（越低越好）
```

直觉上：perplexity = 模型在这段文本上"平均每步有多少种可能性"。perplexity=100 意味着模型平均每步面临 100 种等可能的选择；perplexity=10 意味着只有 10 种。
...
实现上用的是 C++ 的 `getpy.Dict`（比 Python dict 节省约 3 倍内存），每个 kv 对占 9 字节。每个分片约占 2GB RAM。这是一个精确的哈希表，不是布隆过滤器，不存在误判。

**getpy 的卖点**：Python 原生 dict 对每个 key-value 对有大量对象开销（引用计数、类型信息、内存对齐等），一个 `{int: int}` 的 Python dict 实际每对占约 200 字节。getpy 是 C++ 实现的紧凑哈希表，直接按类型（uint64 → uint8）存储，每对只占 9 字节，内存节省约 20 倍（相对 Python dict，不是 3 倍——3 倍是相对于另一种优化实现）。代价是略慢，且类型固定。
...
**分片加载的读写模式**：这个 dict 的访问模式其实不是随机的，而是**顺序扫描**：

- Pass 1（写）：顺序读取所有文档，把每行的哈希 `add` 进 dict，标记"见过"。整个过程是顺序写入，每行只写一次。
- Pass 2（读）：再次顺序读取所有文档，查询每行哈希是否在 dict 里且出现多次，决定保留还是删除。整个过程是顺序读取。

两遍都是顺序 I/O，不是随机访问。分片加载也是顺序的：把文档按 ID 分成 N 组，每次把一组对应的哈希表加载进内存，处理完这组文档后卸载，再加载下一组。开销主要是内存分配，不是磁盘随机读写。
